...
(defvar flycheck-error-table-mode-map
  (let ((map (make-sparse-keymap)))
    (set-keymap-parent map flycheck-error-list-mode-map)
    map)
...
(defun flycheck-error-tree--common-prefix (&rest strings)
  "Returns the longest common prefix of STRINGS."
...
If PRESERVE-POS is nil, recenter the error list on the first error."
  ...
  (let ((beg (point-max)))
    ...
    (when (and (not preserve-pos) (< beg (point-max)))
      (flycheck-error-list-recenter beg))))
...
Errors are taken from `flycheck-error-list-source-buffer'."
  (let ((inhibit-read-only t)
    ...
    (let* ((err (car (member current-error new-errors)))
      ...
      (flycheck-error-list-recenter pos))))
